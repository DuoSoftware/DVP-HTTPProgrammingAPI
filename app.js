...
var PublishDVPEventsMessage = require("./DVPEventPublisher")
  .PublishDVPEventsMessage;
    ...
    if (err) {
      ...
      try {
        if (
          ...
          config.Services.uploadport
        ) {
          var urloadurl = format(
            "http://{0}/DVP/API/{1}/FileService/File/Upload",
            config.Services.uploadurl,
          ...
          if (
            ...
            validator.isIP(config.Services.uploadurl)
          )
            urloadurl = format(
              "http://{0}:{1}/DVP/API/{2}/FileService/File/Upload",
              config.Services.uploadurl,
              config.Services.uploadport,
              config.Services.uploadurlVersion
            );
          ...
          var fileID = format(
            "http://{0}/DVP/API/{1}/InternalFileService/File/DownloadLatest/{2}/{3}/{4}",
            config.Services.downloadurl,
            config.Services.downloaddurlVersion,
            uuid_data["tenant"],
            uuid_data["company"],
            FormData.filename
          );
          ...
          if (
            req.body &&
            req.body["Caller-Caller-ID-Number"] &&
          ...
          logger.debug(
            `Fileservice request data ----> ${FormData.sessionid}, ${FormData.filename}`
          );

          var r = request.post(request_data, function (error, response, body) {
            if (err) {
                  ...
                  if (
                    ...
                    try {
                      ...
                      CreateTicket(
                        ...
                        uuid_data["tenant"],
                        voicemailData["type"],
                        voicemailData["subject"],
                        voicemailData["description"],
                        voicemailData["priority"],
                        voicemailData["tags"],
                        undefined,
                    ...
                    logger.error(
                      "Create engagement no necessary data found ...."
                    );
                  }
                } else {
          ...
          PublishDVPEventsMessage("FILEUPLOADED", {
            Type: "FILE",
...
function Operation(
  ...
  switch (callData["action"]) {
    case "play":
      ...
      if (callData["maxdigits"]) maxdigits = callData["maxdigits"];

      res.write(
        messageGenerator.Playback(
          fileID,
          mainServer,
          mainServer,
          callData["result"],
          callData["errorfile"],
          callData["digittimeout"],
          callData["inputtimeout"],
          callData["loops"],
          callData["terminator"],
          callData["strip"],
          callData["digits"],
    ...
    case "playandgetdigits":
      ...
      if (callData["errorfile"]) error = callData["errorfile"];

      res.write(
        messageGenerator.PlayAndGetDigits(
          fileID,
          mainServer,
          mainServer,
          callData["result"],
    ...
    case "playanddetectspeech":
      res.write(
        messageGenerator.PlayAndDetectSpeech(
          fileID,
          callData["text"],
          mainServer,
          mainServer,
    ...
    case "setdtmf":
      ...
      if (queryData["variable_switch_r_sdp"]) {
        ...
        if (ismatch && ismatch.length > 0) {
          ...
          var msg = messageGenerator.DTMFType(
            mainServer,
            mainServer,
            dtmfPayload
          );
    ...
    case "dialdirect":
      ...
      if (uuid_data["pbxcontext"]) var context = uuid_data["pbxcontext"];
      res.write(
        messageGenerator.Dial(
          mainServer,
          mainServer,
    ...
    case "break":
      ...
      res.write(
        messageGenerator.Break(mainServer, mainServer, callData["cause"])
      );
    ...
    case "ards":
      ...
      res.write(
        messageGenerator.ARDS(
          mainServer,
          mainServer,
          callData["skill"],
          callData["skilldisplay"],
          callData["company"],
          callData["tenant"],
          callData["MOH"],
          callData["FirstAnnounement"],
    ...
    case "getvar":
      ...
      try {
        var permenant = "true";
        if (callData["permanent"]) permenant = callData["permanent"];
        doc = messageGenerator.GetVar(
          mainServer,
          mainServer,
          permenant,
          callData["name"]
    ...
    case "hangup":
      res.write(
        messageGenerator.Hangup(mainServer, mainServer, callData["cause"])
      );
    ...
    default:
      res.write(
        messageGenerator.Hangup(mainServer, mainServer, "NO_ROUTE_DESTINATION")
      );
  ...
  switch (callData["action"]) {
    case "play":
      ...
      debugdata.push({
        type: "action",
        action: "play",
        info: "Play file started wait for terminator or timeout",
        data: callData,
      });
    ...
    case "playandgetdigits":
      ...
      debugdata.push({
        ...
        info: "Play and get digit file started wait for terminator or timeout",
        data: callData,
      });
    ...
    case "record":
      ...
      debugdata.push({
        type: "action",
        action: "record",
        info: "record file started wait for terminator or timeout",
        data: callData,
      });
    ...
    case "speak":
      ...
      debugdata.push({
        type: "action",
        action: "speak",
        info: "speak started wait for terminator or timeout",
        data: callData,
      });
      break;
    ...
    case "say":
      ...
      debugdata.push({
        type: "action",
        action: "say",
        info: "say started wait for terminator or timeout",
        data: callData,
      });
      break;
    ...
    default:
      debugdata.push({
        type: "action",
        action: "hangup",
        info: "Unknown command found from client application so drop call",
        data: callData,
      });
...
function GetUserAttributes(company, tenant, id, attribute, cb) {
  if (
    ...
    var userserviceURL = format(
      "http://{0}/DVP/API/{1}/ExternalUser/" + id + "/attribute/" + attribute,
      config.Services.userserviceurl,
      config.Services.userserviceversion
    );
    if (
      config.Services.dynamicPort ||
      validator.isIP(config.Services.userserviceurl)
    )
      ...
      function (_error, _response, datax) {
        try {
          if (
            ...
            logger.error(
              "There is an error in  getting user attributes for this id " + id
            );
            cb(false, {});
          }
        } catch (excep) {
...
function CreateEngagement(
  ...
  if (
    ...
    var engagementURL = format(
      "http://{0}/DVP/API/{1}/EngagementSessionForProfile",
      config.Services.interactionurl,
      config.Services.interactionversion
    );
    if (
      config.Services.dynamicPort ||
      validator.isIP(config.Services.interactionurl)
    )
      engagementURL = format(
        "http://{0}:{1}/DVP/API/{2}/EngagementSessionForProfile",
        config.Services.interactionurl,
        config.Services.interactionport,
        config.Services.interactionversion
      );
...
function CreateTicket(
  ...
  if (
    config.Services &&
    config.Services.ticketurl &&
    config.Services.ticketport &&
    config.Services.ticketversion
  ) {
    var ticketURL = format(
      "http://{0}/DVP/API/{1}/Ticket",
      config.Services.ticketurl,
      config.Services.ticketversion
    );
    if (
      config.Services.dynamicPort ||
      validator.isIP(config.Services.ticketurl)
    )
      ticketURL = format(
        "http://{0}:{1}/DVP/API/{2}/Ticket",
        config.Services.ticketurl,
        config.Services.ticketport,
        config.Services.ticketversion
      );
      ...
      function (_error, _response, datax) {
        try {
          if (
            ...
            logger.error(
              "There is an error in  create ticket for this session " + session
            );
...
function CreateSubmission(
  ...
  if (
    config.Services &&
    config.Services.csaturl &&
    config.Services.csatport &&
    config.Services.csatversion
  ) {
    //console.log("CreateSubmission start");
    var ticketURL = format(
      "http://{0}/DVP/API/{1}/CustomerSatisfaction/Submission/ByEngagement",
      config.Services.csaturl,
      config.Services.csatversion
    );
    if (config.Services.dynamicPort || validator.isIP(config.Services.csaturl))
      ticketURL = format(
        "http://{0}:{1}/DVP/API/{2}/CustomerSatisfaction/Submission/ByEngagement",
        config.Services.csaturl,
        config.Services.csatport,
        config.Services.csatversion
      );
      ...
      function (_error, _response, datax) {
        ...
        try {
          if (
            ...
            logger.error(
              "There is an error in  create csat for this session " + session
            );
...
function CreateComment(
  ...
  if (
    ...
    var url = format(
      "http://{0}/DVP/API/{1}/TicketByEngagement/{2}/Comment",
      config.Services.ticketurl,
      config.Services.ticketversion,
      engagement._id
    );
    if (
      ...
      url = format(
        "http://{0}:{1}/DVP/API/{2}/TicketByEngagement/{3}/Comment",
        config.Services.ticketurl,
        config.Services.ticketport,
        config.Services.ticketversion,
        engid
...
function AddNoteToEngagement(company, tenant, session, body) {
  if (
    ...
    var engagementURL = format(
      "http://{0}/DVP/API/{1}/EngagementSession/{2}/Note",
      config.Services.interactionurl,
      config.Services.interactionversion,
      session
    );
    if (
      ...
      engagementURL = format(
        "http://{0}:{1}/DVP/API/{2}/EngagementSession/{3}/Note",
        config.Services.interactionurl,
        config.Services.interactionport,
        config.Services.interactionversion,
        session
      ...
      function (_error, _response, datax) {
        try {
          if (!_error && _response && _response.statusCode == 200) {
            ...
            logger.error(
              "There is an error in  add note to engagement " + session
            );
          }
        } catch (excep) {
...
function GetAgentForRequest(company, tenant, sessionID, Attributes, cb) {
  if (
    config.Services &&
    config.Services.ardsServiceHost &&
    config.Services.ardsServicePort &&
    config.Services.ardsServiceVersion
  ) {
    ///http://ardsliteservice.app.veery.cloud/DVP/API/1.0.0.0/ARDS/request

    var ardsURL = format(
      "http://{0}/DVP/API/{1}/ARDS/request",
      config.Services.ardsServiceHost,
      config.Services.ardsServiceVersion
    );
    if (
      config.Services.dynamicPort ||
      validator.isIP(config.Services.ardsServiceHost)
    )
      ardsURL = format(
        "http://{0}:{1}/DVP/API/{2}/ARDS/request",
        config.Services.ardsServiceHost,
        config.Services.ardsServicePort,
        config.Services.ardsServiceVersion
      );
      ...
      function (_error, _response, datax) {
        try {
          if (!_error && _response && _response.statusCode == 200) {
            if (
              _response.body &&
              _response.ResourceInfo &&
              _response.ResourceInfo.ResourceId
            ) {
...
function AddToInbox(
  ...
  if (
    ...
    var engagementURL = format(
      "http://{0}/DVP/API/{1}/Inbox/Message",
      config.Services.interactionurl,
      config.Services.interactionversion
    );
    if (
      ...
      engagementURL = format(
        "http://{0}:{1}/DVP/API/{2}/Inbox/Message",
        config.Services.interactionurl,
        config.Services.interactionport,
        config.Services.interactionversion
      );
...
function HandleFunction(queryData, req, res, next) {
  ...
  redisClient.get(queryData["session_id"] + "_data", function (
    err,
    sessiondata
  ) {
    var uuid_dev;
    if (err) {
      logger.error(
        "HTTPProgrammingAPI.Handler REDIS Error in sessiondata retrieve %s",
      ...
      if (!sessiondata) {
        ...
        logger.debug(
          "HTTPProgrammingAPI.Handler no sessiondata found create new"
        );
      }
      ...
      if (uuid_data && !uuid_data.taken) {
        logger.info(
          "Session is going to reset ----------------------------------------------------------------------------------->"
        );
        reset = true;
        uuid_data.taken = true;
        redisClient.set(
          queryData["session_id"] + "_data",
          JSON.stringify(uuid_data),
          redis.print
        );
        logger.info(
          "Session is going to reset -----------------------------------------------------------------------------------> done"
        );
      }
      ...
      if (!uuid_data) {
        ...
        redisClient.get(queryData["session_id"] + "_dev", function (
          err,
          value
        ) {
          var uuid_dev;
          if (err) {
            logger.error(
              "HTTPProgrammingAPI.Handler REDIS Error in sessiondev retrieve %s",
              queryData["session_id"],
              err
            );
            res.writeHead(200, { "Content-Type": "text/xml" });
            ...
            if (
              queryData["Caller-Channel-Name"] &&
              queryData["Caller-Channel-Name"].indexOf("@sip.skype.com") !== -1
            ) {
            ...
            if (!value) {
              ...
              logger.debug(
                "HTTPProgrammingAPI.Handler Session Create %s",
                queryData["session_id"]
              );
              ...
              logger.info(
                "Session is going to reset -----------------------------------------------------------------------------------> inside"
              );
              if (reset == true) {
              ...
              logger.info(
                "Session is going to reset -----------------------------------------------------------------------------------> outside"
              );
            ...
            CreateEngagement(
              ...
              uuid_data["tenant"],
              callerID,
              queryData["Caller-Destination-Number"],
              queryData["Caller-Direction"],
              queryData["session_id"],
              undefined,
              function (isSuccess, result) {
                if (isSuccess && result) {
...

                {
                  if (queryData["ARDS-Resource-Profile-Name"]) {
                    uuid_dev["resource"] =
                      queryData["ARDS-Resource-Profile-Name"];
                  }
                  ...
                  if (queryData["detect_speech_result"]) {
                    try {
                      ...
                      if (queryData["detect_speech_result"]) {
                        try {
                          detected_result = JSON.parse(
                            convert.xml2json(
                              queryData["detect_speech_result"],
                              {
                                compact: true,
                                spaces: 4,
                              }
                      ...
                      if (
                        ...
                        detected_result.result.interpretation &&
                        detected_result.result.interpretation.input &&
                        detected_result.result.interpretation.input._text
                      ) {
                        queryData["detect_speech_result_string"] =
                          detected_result.result.interpretation.input._text;
                        logger.info(
                          "Detected voice : " +
                            detected_result.result.interpretation.input._text
                  ...
                  logger.debug(
                    "HTTPProgrammingAPI.Handler RequestOut DeveloperAPP Data %s %j",
                    queryData["session_id"],
                    body
                  );
                  ...
                  logger.debug(
                    "HTTPProgrammingAPI.Handler REDIS Publish data to event flow %s %j",
                    queryData["session_id"],
                    callreciveEvent
                  );
                  ...
                  ////////////////////////////////////////
                  logger.debug(
                    "HTTPProgrammingAPI.Handler RequestOut DeveloperAPP URL %s",
                    uuid_dev["nexturl"]
                  );
                  ...
                  request(options, function (error, response, data) {
                    if (queryData["exiting"] == "true") {
                      logger.debug(
                        "HTTPProgrammingAPI.Handler Session Leave %s",
                        queryData["session_id"]
                      );
                      ...
                      logger.debug(
                        "HTTPProgrammingAPI.Handler REDIS Publish data to event flow %s",
                        queryData["session_id"],
                        callreciveEvent
                      );
                      //////////////////////////////////////////////////////////////////////////////// %s///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                      ...
                      if (!error && response.statusCode == 200) {
                        logger.debug(
                          "HTTPProgrammingAPI.Handler RequestOut DeveloperAPP Success %s %j",
                          queryData["session_id"],
                          response.body
                        );
                        ...
                        logger.debug(
                          "HTTPProgrammingAPI.Handler REDIS lpush data for command %s",
                          queryData["session_id"]
                        );
                        ...
                        try {
                          ...
                          if (callData.params) {
                            ...
                            Object.keys(callData.params).forEach(function (
                              key
                            ) {
                          ...
                          logger.debug(
                            "HTTPProgrammingAPI.Handler REDIS Publish data for monitoring api %s %j",
                            queryData["session_id"],
                            eventFlowData
                          );
                          var date = new Date();
                          ...
                          res.end();
                          logger.error(
                            "HTTPProgrammingAPI.Handler RequestOut DeveloperAPP DataError %s %j",
                            queryData["session_id"],
                            callData
                          );
                        ...
                        if (
                          callData["action"] == "play" ||
                          callData["action"] == "playandgetdigits" ||
                          callData["action"] == "playanddetectspeech"
                        ) {
                          //console.log("inplay.....");
                          ...
                          if (filenamex) {
                            if (
                              config.Services &&
                              config.Services.fileserviceurl &&
                              ...
                              urlx = format(
                                "http://{0}/DVP/API/{1}/FileService/File/{2}/ofApplication/{3}",
                                config.Services.fileserviceurl,
                              ...
                              if (
                                config.Services.dynamicPort ||
                                validator.isIP(config.Services.fileserviceurl)
                              )
                                urlx = format(
                                  "http://{0}:{1}/DVP/API/{2}/FileService/File/{3}/ofApplication/{4}",
                                  config.Services.fileserviceurl,
                                  config.Services.fileserviceport,
                                  config.Services.fileserviceVersion,
                                  filenamex,
                              ...
                              logger.debug(
                                `Calling FILE service -----> ${JSON.stringify(
                                  request_data
                                )} `
                              );
                              request.get(request_data, function (
                                _error,
                                _response,
                                datax
                              ) {
                                ...
                                try {
                                  ...
                                  if (
                                    ...
                                    filedata &&
                                    filedata.Result &&
                                    filedata.Result["UniqueId"]
                                  ) {
                                    logger.debug(
                                      "HTTPProgrammingAPI.Handler Request File resolution Responsedata %d %j %j ",
                                      _response.statusCode,
                                      filedata,
                                      filedata.Result
                                    );

                                    var ext = filedata.Result.FileStructure.split(
                                      /[/]+/
                                    ).pop();
                                    fileID = format(
                                      "{0}.{1}",
                                    ...
                                    if (
                                      ...
                                      fileID = format(
                                        "http://{0}:{1}/DVP/API/{2}/InternalFileService/File/DownloadLatest/{3}/{4}/{5}",
                                        config.Services.downloadurl,
                                        config.Services.downloadport,
                                        config.Services.downloaddurlVersion,
                                        uuid_data["tenant"],
                                        uuid_data["company"],
                                        filenamex
                                    ...
                                    logger.debug(
                                      "HTTPProgrammingAPI.Handler Request File resolution %s %s",
                                      queryData["session_id"],
                                      fileID
                                    );
                                  } else {
                                    //uuid_data["tenant"],uuid_data["company"]
                                    var companyLocation = format(
                                      "{0}/{1}",
                                      uuid_data["tenant"],
                                      uuid_data["company"]
                                    );
                          ...
                          if (
                            config.Services &&
                            config.Services.ruleservice &&
                            config.Services.ruleserviceport
                          ) {
                            //, uuid_data["tenant"],uuid_data["company"]

                            outbountruleurl = format(
                              "http://{0}/DVP/API/{1}/CallRule/Outbound/ANI/{2}/DNIS/{3}",
                              config.Services.ruleservice,
                            ...
                            if (
                              config.Services.dynamicPort ||
                              validator.isIP(config.Services.ruleservice)
                            )
                              outbountruleurl = format(
                                "http://{0}:{1}/DVP/API/{2}/CallRule/Outbound/ANI/{3}/DNIS/{4}",
                                config.Services.ruleservice,
                                config.Services.ruleserviceport,
                                config.Services.ruleserviceVersion,
                                callData["callernumber"],
                            ...
                            function (_error, _response, datax) {
                              ...
                              try {
                                if (
                                  ...
                                  logger.debug(
                                    "HTTPProgrammingAPI.Handler Request Gateway resolution %s %j",
                                    queryData["session_id"],
                                    ruledata
                                  );
                                } else {
                                  logger.error(
                                    "HTTPProgrammingAPI.Handler Request Gateway resolution %s",
                                    queryData["session_id"]
                                  );
                          ...
                          if (config.Services && config.Services.ards) {
                            ...
                            if (
                              config.Services.dynamicPort ||
                              validator.isIP(config.Services.ards)
                            )
                              queueURL = format(
                                "http://{0}:{1}/ardsurl/{2}/{3}",
                                config.Services.ards,
                                config.Services.ardsport,
                                uuid_data["tenant"],
                                uuid_data["company"]
                              );
                          }
                            ...
                            function (_error, _response, datax) {
                              try {
                                if (
                                  ...
                                  logger.debug(
                                    "HTTPProgrammingAPI.Handler Request Queue resolution %s %j",
                                    queryData["session_id"],
                                    urldata
                                  );
                                } else {
                                  ...
                                  logger.error(
                                    "HTTPProgrammingAPI.Handler Request Queue resolution %s",
                                    queryData["session_id"]
                                  );
                                }
                        ...
                        /////////////////////////////////////////ards command////////////////////////////////////////////////////////////////////////////
                          ...
                          if (config.Services && config.Services.qmusicurl) {
                            profileURL = format(
                              "http://{0}/DVP/API/{1}/QueueMusic/Profile/{2}",
                              config.Services.qmusicurl,
                              config.Services.qmusicVersion,
                              callData["profile"]
                            );

                            if (
                              config.Services.dynamicPort ||
                              validator.isIP(config.Services.qmusicurl)
                            )
                              profileURL = format(
                                "http://{0}:{1}/DVP/API/{2}/QueueMusic/Profile/{3}",
                                config.Services.qmusicurl,
                                config.Services.qmusicport,
                                config.Services.qmusicVersion,
                                callData["profile"]
                              );
                          }
                        ...
                        if (response) {
                          ...
                          logger.debug(
                            "HTTPProgrammingAPI.Handler REDIS Publish error for monitoring api %s %j",
                            queryData["session_id"],
                            callreciveEvent
                          );
                        } else {
                        ...
                        logger.error(
                          "HTTPProgrammingAPI.Handler RequestOut %s with response",
                          queryData["session_id"]
                        );
  ...
  redisClient.get(queryData["session_id"] + "_data", function (
    ...
    if (err) {
      ...
      if (!uuid_data) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.write(
          JSON.stringify({ info: "Call Drop NODATA for given session" })
        );
        res.end();
      } else {
        redisClient.get(queryData["session_id"] + "_dev", function (
          ...
          if (err) {
            res.writeHead(200, { "Content-Type": "application/json" });
            res.write(
              JSON.stringify({ info: "Call Drop NODEVDATA for given session" })
            );
            res.end();
          } else {
            uuid_dev = JSON.parse(value);
            ...
            request(options, function (error, response, data) {
              if (!error && response.statusCode == 200) {
                ...
                debugdata.push({
                  type: "message",
                  info: "Receive data from client app",
                  data: {
                    url: uuid_dev["nexturl"],
                ...
                try {
                  ...
                  debugdata.push({
                    type: "error",
                    info: "Data recived not in correct format",
                    error: e,
                  });
                ...
                if (
                  ...
                  if (
                    config.Services.dynamicPort ||
                    validator.isIP(config.Services.downloadurl)
                  )
                    url = format(
                      "http://{0}:{1}/{2}/GetFileIDForName/{3}",
                      config.Services.downloadurl,
                      config.Services.downloadport,
                      filenamex,
                      uuid_data["appid"]
                ...
                if (
                  ...
                  if (
                    ...
                    if (
                      ...
                      outbountruleurl = format(
                        "http://{0}:{1}/Outbound/ANI/{2}/DNIS/{3}",
                        config.Services.ruleservice,
                        config.Services.ruleserviceport,
                        callData["callernumber"],
                        callData["number"]
...
server.post("/routex", function (req, res, next) {
  ...
  var params = format(
    "{originate_timeout=20,return_ring_ready=true,Originate_session_uuid={0}{1}",
    data.SessionID,
    "}"
  );
