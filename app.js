                                        ...
                                        request.get({url:urlx, headers: {authorization: token, companyinfo: format("{0}:{1}",uuid_data["tenant"],uuid_data["company"])}},function (_error, _response, datax) {
                                                ...
                                                try {
                                                    ...
                                                    if (!_error && _response && _response.statusCode == 200 && filedata && filedata.Result && filedata.Result["UniqueId"]) {
                                                        ...
                                                        fileID = format("http://{0}/DVP/API/{1}/InternlFileService/File/DownloadLatest/{2}/{3}/{4}", config.Services.downloadurl, config.Services.downloaddurlVersion, uuid_data["tenant"], uuid_data["company"], filenamex);


                                                        if (validator.isIP(config.Services.downloadurl))
                                                            fileID = format("http://{0}:{1}/DVP/API/{2}/InternlFileService/File/DownloadLatest/{3}/{4}/{5}", config.Services.downloadurl, config.Services.downloadport, config.Services.downloaddurlVersion, uuid_data["tenant"], uuid_data["company"], filenamex);
